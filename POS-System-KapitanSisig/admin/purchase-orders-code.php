...
if (isset($_POST['addIngredient'])) {
    ...
    $checkIngredient = mysqli_query($conn, "
        SELECT i.*, u.uom_name AS unit_name 
        FROM ingredients i
    ...
    if ($checkIngredient) {
        if (mysqli_num_rows($checkIngredient) > 0) {
            ...
            $ingredientData = [
                ...
                'price' => $row['price'], // Ensure this is valid in your context
                'quantity' => $quantity,
            ];
...
if (isset($_POST['proceedToDeliveredIng'])) {
    ...
    $query = "SELECT id FROM purchase_orders WHERE tracking_no = '$order_track' LIMIT 1"; // Make sure the table name is correct
    ...
    if ($orderData) {
        ...
        $updateResult = update('purchase_orders', $order_id, $data); // Ensure the correct table name
...
if (isset($_POST['savePurchaseOrder'])) {
    ...
    if (mysqli_num_rows($checkAdmin) > 0) {
        if (mysqli_num_rows($checkSupplier) > 0) {
            ...
            $result = insert('purchase_orders', $data); // Ensure the correct table name
